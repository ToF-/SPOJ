...
main = hspec $ do
    describe "adjacent" $ do
        describe "find distances of cells adjacent to a cell" $ do
            it "on a single cell" $ do
                let initial = fromList [(2,2)-:3]
                    visited = []
                    result  = sort $ toList $ fst $ adjacent initial visited (10,10)
            ...
            it "within the dimensions" $ do
                let initial = fromList [(0,0)-:3]
                    visited = []
                    result  = sort $ toList $ fst $ adjacent initial visited (3,3)
        ...
        it "yields an updated list of distances" $ do
            let initial = fromList [(7,2)-:6, (2,2)-:3]
                visited = []
                result  = sort $ snd $ adjacent initial visited (10,10)
                expected = sort [(2,1)-:4, (2,2)-:3, (2,3)-:4, (1,2)-:4, (3,2)-:4, (7,2)-:6] 
            result `shouldBe` expected
