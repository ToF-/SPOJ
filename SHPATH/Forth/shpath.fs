...
: READ-INPUT-LINE ( -- count )
    LINE-BUFFER LINE-MAX INPUT-FILE @ READ-LINE THROW DROP ;
    
: READ-EDGES ( n -- )
    0 DO
        READ-INPUT-LINE                  ( count )
        LINE-BUFFER SWAP STR-TOKENS      ( add1,count1,add2,count2,2 )
        ASSERT( 2 = )
...
: READ-NODE
    READ-INPUT-LINE
    LINE-BUFFER SWAP STR-TOKENS ASSERT( 1 = )
    INSERT-NODE
    READ-INPUT-LINE
    LINE-BUFFER SWAP STR-TOKENS ASSERT( 1 = )
    STR>NUMBER
...
: READ-TEST-CASE
    ...
    READ-INPUT-LINE
    STR-TOKENS ASSERT( 1 = )
    STR>NUMBER 0 DO READ-NODE LOOP
    READ-REQUESTS ;
